...
<?php
    $title = "Convert | Convert Application";
    ...
    if(isset($_REQUEST['multiple_submit']))
    {
        if($_SESSION['user_login_detail']['id'] == $_REQUEST['created_by'])
        {
            if(isset($_REQUEST['id']) && isset($_REQUEST['multiple']))
            {
                ...
                $return = update_data('`convert_data`', $data, $conditions, $conn);
    ...
    if(isset($_REQUEST['d_id']) && isset($_REQUEST['c_id']))
    {
        if($_SESSION['user_login_detail']['id'] == $_REQUEST['c_id'])
        {
            ...
            $return = delete_data('`convert_data`', $conditions, $conn);
...
<script>
    function add_new()
    {
        ...
        {
            window.location.href = "<?= base_url ?>admin/convert/add.php";
        }
    }
    function edit(id, created_by)
    {
        if(confirm('Are you sure?'))
        {
            window.location.href = "<?= base_url ?>admin/convert/edit.php?e_id="+id;
        }
    }
    function deletes(id, created_by)
    {
        if(confirm('Are you sure?'))
        {
            window.location.href = "<?= base_url ?>admin/convert/index.php?d_id="+id+"&c_id="+created_by;
        }
    }
